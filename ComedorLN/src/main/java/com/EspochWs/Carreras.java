...
public class Carreras {
    ...
    public String getRolUsuarioCarrera(String cedula) {
        String Respuesta = "";
        String texto = "";
        String url = "http://academico.espoch.edu.ec/OAS_Interop/Seguridad.asmx?op=GetRolUsuarioCarrera";
        ...
        try {
            ...
            String xml = "<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:acad=\"http://academico.espoch.edu.ec/\">\n"
                    ...
                    + "   </soapenv:Header>\n"
                    + "   <soapenv:Body>\n"
                    + "      <acad:GetRolUsuarioCarrera>\n"
                    + "         <!--Optional:-->\n"
                    + "         <acad:login>172179606-6</acad:login>\n"
                    + "      </acad:GetRolUsuarioCarrera>\n"
                    + "   </soapenv:Body>\n"
                    + "</soapenv:Envelope>";
            ...
            while ((inputLine = in.readLine()) != null) {
                ...
                System.out.println("responsed " + responsed.toString());
            ...
            System.out.println("com.EspochWs.Carreras.getRolUsuarioCarrera() " + ex);
        }

        return "HOLA";
        //return "{ \"carreras\" : " + xmlToJsonRolusuario(Respuesta) + "}";
        //return "{ \"carreras\" : " + xmlToJson(Respuesta) + "}";
    }
    ...
    private String xmlToJsonRolusuario(String Respuesta){
        
        System.err.println("xmlToJsonRolusuario " + Respuesta);
        JSONObject json = XML.toJSONObject(Respuesta);

        json = json.getJSONObject("soap:Envelope").getJSONObject("soap:Body").getJSONObject("GetRolUsuarioCarreraResponse");

        String datosCarreras;
        if (!json.isNull("GetRolUsuarioCarreraResult")) {
            //datosCarreras = json.getJSONObject("GetTodasCarrerasResult").toString();
            //datosCarreras = json.getJSONObject("GetRolUsuarioCarreraResult").getJSONArray("UnidadAcademica").toString();
            datosCarreras = json.getJSONObject("GetRolUsuarioCarreraResult").toString();
            System.out.println(datosCarreras);
        } else {
            datosCarreras = "{\"error\": \"No hay datos\" }";
        }
